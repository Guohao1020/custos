...
- **资源接入是 demo 反面教材**：目标库 `target-jdbc-url` 硬编码在 `application.yml`，高权限管理凭证是
  compose 明文 `custos`/`custospwd`（还 GRANT ALL），`DynamicDbCredentials` 把 MySQL 的
  `CREATE USER 'x'@'%'` 语法写死。无法对接企业一个 fleet 的库 / 中间件 / 系统。
...
都先过 Custos 鉴权→签即用即焚凭证→落审计。具体杠杆点见 §3.1。
...
### 3.1 · Nacos AI 中心杠杆点（源码+live API 实证，按里程碑归位）
...
| **v0.5** | 资源接入 + Agent + Nacos AI 中心 | ① **资源接入(首要)**：资源注册表 + 高权限密钥 Barrier 托管 + 角色/签发模型(hybrid 适配器+模板) + 多 DB 方言(见 §4.2)② AK·SK/KV/PG 经 MCP 多工具暴露 ③ custos MCP server 注册进 Nacos 3.2 AI 管理中心 ④ REQUIRE_APPROVAL 审批闭环 ⑤ 真 Claude/Codex 端到端实证 + 进 CI ⑥ Python SDK | 🔜 下一阶段 |
...
- **custos 侧（自有数据）**：防篡改审计链浏览、seal 状态、活跃租约、动态账号、决策/拒绝速率、审批队列、运维动作、**资源配置 GUI（注册资源、填/轮换高权限密钥、看活跃签发）**。
- **Nacos 侧（读取呈现）**：当前生效策略、已注册 MCP 工具、已注册 custos 服务实例、资源元数据。

需要新增只读 REST（审计分页查询、租约列表、监控统计）——这些是 v0.6 的实现项，本 spec 只登记。

### 4.2 · 资源接入模型（v0.5 首要，本次仅规划）
...
Hero + slogan + **主线一句话**（Nacos 发现，Custos 执行 —— §2.1）→ 痛点（Agent 要密钥，但你不能把密钥交给它）→
四大护城河（§2）→ 工作原理（一次 query_db 的 secretless 流程图 + Nacos 发现→Custos PEP 的位置图）→
**接入你的资源 fleet**（后台注册库/中间件 + 高权限密钥托管 → Agent 拿短时凭证，标 roadmap）→
对标 Vault/OpenBao 表 → **shipped vs roadmap 诚实带**（数据取自 §4）→ CTA（`docker compose up` / GitHub）。
...
- 不在本次实现资源接入、Admin Console、多引擎暴露、Python SDK、审批闭环——它们是 ROADMAP 待建项，各自后续 spec→plan（资源接入是 v0.5 首个，将单独立 spec 细化资源/角色/密钥托管数据模型）。
